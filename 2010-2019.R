...
#[1]  0.0900000  0.2700000  0.2590980  0.1113529  0.6295490  0.0000000 -1.2025549 -0.1378612
library(ggplot2)
table_plot = NULL
...
p1 <-ggplot(table_plot, aes(x=value, color=type)) +
  geom_histogram(aes(y=..density..), position="identity", alpha=0.1, fill="white" , binwidth=0.18, size = 0.5)+
...
p = length(Z)
library(RSpectra)
eig = eigs_sym(Sigma, 200, which = "LM")
# calculate posterior prob(mu <= 0 | Z)
set.seed(20200711)

eig_value = 0.6
sim_size =3000
prob_1 = d_value(Z, Sigma, eig, eig_value, sim_size, best_set)
prob_2 = l_value(Z, Sigma, eig, eig_value, sim_size, best_set)

level = 0.3
decision_p = Optimal_procedure_3(prob_1,level)
decision_p$cj



#fund_analysis_2009_2018_E.csv
fund_analysis = data.frame(crsp_fundno = S, alpha  = alpha, Z = Z, sharpe = fund_ana2,prob_1 = prob_1,
                           prob_2 = prob_2, tret_set = tret_set,R_set = R_set)

#save(fund_analysis, file = "~/Desktop/OPTtesing_data/fund_analysis_2010_2019_E.csv")
